...
enum DIRECTION{UP,DOWN,LEFT,RIGHT,NONE}; //none=fire
...
int main()
{
  ...
  initializeInvaders(invaders); //modify invaders structure
...
//  // http://www.libsdl.org/projects/SDL_image/
  ...
  {
    printf("IMG_Load: %s\n", IMG_GetError());
    return EXIT_FAILURE; // we need to convert the image in a texture
  }
...
//  // once we have the texture it will be store in hardware and we don't need the image data anymore
  ...
  // free the image, we dont need it anymore
  ...
  SDL_SetRenderDrawColor(ren, 0, 0, 0, 255); //color of the screen
  SDL_RenderClear(ren);
  updateInvaders(invaders);
  drawInvaders(ren,tex,invaders); //renderer, texture of the invaders
...
void initializeInvaders(Invader invaders[ROWS][COLS])
{
  ...
  {
    int xpos=GAP;
    for(int c=0; c<COLS; ++c)//crate a rectangle, have a gap, crate, gap, etc...
    {
      pos.x=xpos+SPRITEWIDTH;
      pos.y=ypos+SPRITEHEIGHT; //position
      xpos+=(GAP+SPRITEWIDTH);
      invaders[r][c].pos=pos;
      invaders[r][c].active=1; //alive
      ...
      if(r==0)
        ...
        invaders[r][c].type=TYPE3; //different types of aline depending on the row
...
void drawInvaders(SDL_Renderer *ren, SDL_Texture *tex, Invader invaders[ROWS][COLS])
{
  ...
  for(int r=0; r<ROWS; ++r)
  {
    for(int c=0; c<COLS; ++c)
    {
      ...
      {
      case TYPE1 : SDL_SetRenderDrawColor(ren, 255, 0, 0, 255); break; //red
      case TYPE2 : SDL_SetRenderDrawColor(ren, 0, 255, 0, 255); break; //green
      case TYPE3 : SDL_SetRenderDrawColor(ren, 0, 0, 255, 255); break; //blue
      }
...
{
  enum DIR{FWD,BWD};
  static int DIRECTION=FWD; //when the programm strats I am moving forward
  int yinc=0;
  ...
  for(int r=0; r<ROWS; ++r)
  {
    ...
    {
      if(DIRECTION==FWD)
        invaders[r][c].pos.x+=1; //position forward
      else
        invaders[r][c].pos.x-=1; //going back, linked to speed of the invaders
